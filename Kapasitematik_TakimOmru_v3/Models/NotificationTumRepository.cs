...
{
    public class NotificationTumRepository
    {
        ...
        public List<NotificationModel> NotificationTumList(int sessionId)
        {
            var notification = new List<NotificationModel>();
            using (var cmd = new SqlCommand($@"SELECT [NotificationID], 
 [Notification_Description], [Notification_Date] FROM [dbo].[Notification] where [FKUserId]={sessionId}", con))
            {
