...
namespace Kapasitematik_TakimOmru_v3.Models
{
    public class NotificationRepository
    {
        ...
        public List<NotificationModel> NotificationList(int sessionId)
        {
            var notification = new List<NotificationModel>();
            using (var cmd = new SqlCommand($@"SELECT [NotificationID], 
 [Notification_Description], [Notification_Date] FROM [dbo].[Notification] where [FKUserId]={sessionId} AND [Notification_Date]>DATEADD(DAY,-7,GETDATE())", con))
            {
